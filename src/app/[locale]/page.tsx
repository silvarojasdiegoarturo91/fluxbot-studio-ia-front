  ...
  return (
    <div className="pb-12 pt-8 text-[#173b4d] md:pt-14">
      ...
      <section className="grid gap-5 py-16 md:grid-cols-3">
        {pillars.map(([title, detail], index) => <article key={title} className="rounded-3xl border border-[#173b4d]/10 bg-white p-7"><span className="text-sm font-semibold text-[#d9654b]">0{index + 1}</span><h2 className="mt-5 text-2xl font-semibold tracking-tight">{title}</h2><p className="mt-3 leading-7 text-[#526a76]">{detail}</p></article>)}
      </section>

      <section className="rounded-[2rem] bg-[#173b4d] px-7 py-12 text-white md:px-12"><div className="grid gap-10 lg:grid-cols-[.8fr_1.2fr]"><div><p className="text-sm font-semibold uppercase tracking-[0.16em] text-[#f6bf73]">La hoja de ruta</p><h2 className="mt-4 text-4xl font-semibold tracking-tight">Una IA que se gana la confianza antes de pedirla.</h2><p className="mt-4 leading-7 text-[#d4e0e4]">No vendemos otra caja negra. Construimos las seis capas que hacen que una conversación sea útil para la persona y rentable para la tienda.</p></div><ol className="grid gap-3 sm:grid-cols-2">{roadmap.map((item, index) => <li key={item} className="flex items-center gap-3 rounded-2xl border border-white/15 px-4 py-4"><span className="grid h-7 w-7 place-items-center rounded-full bg-[#d9654b] text-xs font-bold">{index + 1}</span><span className="font-medium">{item}</span></li>)}</ol></div></section>

      <section className="grid gap-6 py-16 lg:grid-cols-2"><div><p className="text-sm font-semibold uppercase tracking-[0.16em] text-[#6c8b74]">Para equipos que venden</p><h2 className="mt-4 text-4xl font-semibold tracking-tight">Saber qué pregunta un cliente es útil. Saber qué cambiar después es crecimiento.</h2></div><div className="rounded-3xl border border-[#173b4d]/10 bg-[#eaf1e8] p-7"><ShieldCheck className="h-7 w-7 text-[#426d50]" /><p className="mt-4 text-xl font-medium leading-8">“No puedo confirmar esa compatibilidad. He creado una mejora para que el equipo la revise.”</p><p className="mt-3 text-[#526a76]">El agente protege la confianza del cliente y convierte la incertidumbre en una tarea accionable.</p></div></section>
      <section className="flex flex-col items-start justify-between gap-6 rounded-3xl border border-[#d9654b]/30 bg-[#fff1eb] p-8 md:flex-row md:items-center"><div><h2 className="text-2xl font-semibold">Empieza por una conversación que sí sabe cuándo parar.</h2><p className="mt-2 text-[#526a76]">Instalación guiada, catálogo conectado y control desde el primer día.</p></div><Link href="/es/contact" className="inline-flex items-center gap-2 rounded-full bg-[#173b4d] px-6 py-3 font-semibold text-white">Hablar con el equipo <CheckCircle2 className="h-4 w-4" /></Link></section>
    </div>
  );
}
